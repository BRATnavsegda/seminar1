void LetsGo()
{
    while (true)
    {
        ...
        if (taskNumber == 64)
        {
            Console.WriteLine($"Числа по порядку в указанном диапазоне: {NaturalNumbers(enterNumber(), enterNumber())}");
        }
        else if (taskNumber == 66)
        {
            Console.WriteLine($"Сумма равна: {SumNaturalNums(enterNumber(), enterNumber())}");
        }
        else if (taskNumber == 68)
        {
            Console.WriteLine($"\nЗначение при вычислении функции Аккермана \nдля введённых вами цифр, будет следующим: {Akkerman(enterNumber(), enterNumber())}; ");
        }
